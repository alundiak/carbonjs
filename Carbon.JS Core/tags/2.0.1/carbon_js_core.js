	...
	get: function(where, selector) {
		...
		function getNodes(elements) {
			...
			for (var i = 0; i < elements.length; i++) {
				if (elements[i].nodeType == 1) els.push(elements[i]);
			}
			return els;
		}
			...
			if (part.indexOf(":") != -1 && regs[3].test(part.substring(part.indexOf(":"), part.lastIndexOf("")))) {
				...
				for (var i = 0; i < nopc.length; i++) {
					...
					switch (pseudo) {
						...
						case "only-child":
							for (var j = 0; j < elements.length; j++) {
								if (getNodes(elements[j].parentNode.childNodes).length == 1) buf.push(elements[j]);
							}
							break;
						case "root":
					...
					if (regs[0].test(selectors[i])) {
						...
						for (var j = 0; j < parts.length; j++) {
							switch (combinators[j]) {
								...
								case "~":
									/*for (var k = 0; k < elems.length; k++) {
										var searchFor = QF(elems[k].parentNode, parts[j]);
										var ns = elems[k];
										while ((ns = ns.nextSibling) && !ns.added) {
											for (var l = 0; l < searchFor.length; l++) {
												if (ns == searchFor[l]) {
													ns.added = true;
													buf.push(searchFor[l]);
