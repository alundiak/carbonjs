...
CarbonJS.extend({
	...
	text: function() {
		...
		var text = [], i = 0, ii, nodes = this[0].childNodes;
		while (ii = nodes[i++]) text[text.length] = Q(ii).text(); // Поскольку innerText поддерживается не всеми браузерами, используем DOM-свойства элементов
		return text.join(" ");
	},
